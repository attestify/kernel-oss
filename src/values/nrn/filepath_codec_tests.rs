...
fn encode_success() {
    let nrn =
        NRN::new("nrn:procedure:nape/software-procedures:rust-ci/sourcecode-integration").unwrap();
...
#[test]
fn decode_success() {
    let result = decode_from_directory_name(
        "nrn_procedure_nape_-_software-procedures_rust-ci_-_sourcecode-integration",
    )
    .unwrap();
    assert_eq!(
...
fn decode_error_bad_nrn() {
    ...
    assert!(
        ...
        "{}",
        format!(
            "An error was expected but non error was returned: {:?}",
            result.unwrap()
        )
    );
