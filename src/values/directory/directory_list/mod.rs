...
pub struct DirectoryList {
    /// The named directory and file paths.
...
impl DirectoryList {
    ...
    /// # Errors
    ///
    /// If the vector of **paths** is empty, an [`Error`] is returned for the [`Audience::System`](crate::error::Audience::System) with [`Kind::InvalidInput`] and a message indicating that no directory or file paths have been provided.
    ///
    pub fn try_from_vec(paths: Vec<(String, String)>) -> Result<DirectoryList, Error> {
        if paths.is_empty() {
            return Err(Error::for_system(
                Kind::InvalidInput,
                "No directory or file paths have been provided.".to_string(),
            ));
        }
        Ok(DirectoryList { paths })
    }

    /// Creates a directory list from a hashmap of path names to paths.
    ...
    /// # Errors
    ///
    /// - If the **path name** is an empty string, an [`Error`] is returned for the [`Audience::System`](crate::error::Audience::System) with [`Kind::InvalidInput`] and a message indicating that an empty path name was provided.
    /// - If the **path** is an empty string, an [`Error`] is returned for the [`Audience::System`](crate::error::Audience::System) with [`Kind::InvalidInput`] and a message indicating that an empty path was provided.
    /// - If the **path name** already exists in the [`DirectoryList`], an [`Error`] is returned for the [`Audience::System`](crate::error::Audience::System) with [`Kind::InvalidInput`] and a message indicating that the path name already exists in the directory List.
    ///
    ...
    /// Merges another directory list into this one.
