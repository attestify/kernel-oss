...
fn add_single_activity_success() {
    ...
    assert_eq!(
        ...
        Name::try_from("some-activity").unwrap()
    );
...
fn add_action_to_empty_activities_success() {
    ...
    let evidence_file_1 =
        SignedFile::new("./some-location/file-1.txt", &evidence_file_1_sig).unwrap();
    ...
    assert_eq!(
        activities.list()[0].name,
        Name::try_from("activity-1").unwrap()
    );
...
fn add_action_with_new_activity_success() {
    ...
    assert_eq!(
        ...
        Name::try_from("activity-2").unwrap()
    );
    assert_eq!(activities.action_count(), 1);
...
fn generate_testing_action(name: &str) -> Action {
    Action::builder()
        .name(name)
        .outcome("pass")
        .reason("action reason")
