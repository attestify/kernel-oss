    ...
    fn validate_name(&self) -> Result<Name, Error> {
        match &self.name {
            ...
            None => match &self.name_str {
                Some(name_str) => Name::try_from(name_str).map_err(|e| {
                    Error::for_user(
                        Kind::InvalidInput,
                        format!(
                ...
                None => Err(Error::for_user(
                    Kind::InvalidInput,
                    "Please provide a name for the action.".to_string(),
                )),
            },
        }
    }
    ...
    fn validate_outcome(&self) -> Result<Outcome, Error> {
        match &self.outcome {
            ...
            None => match &self.outcome_str {
                Some(outcome_str) => Outcome::try_from(outcome_str).map_err(|e| {
                    Error::for_user(
                        Kind::InvalidInput,
                        format!(
                            "There is an issue with the outcome '{}'. {}",
                            outcome_str, e.message
                        ),
                    )
                }),
                None => Err(Error::for_user(
                    Kind::InvalidInput,
                    "Please provide an outcome for the action.".to_string(),
                )),
            },
        }
    }
    ...
    fn validate_reason(&self) -> Result<Description, Error> {
        match &self.reason {
            ...
            None => match &self.reason_str {
                Some(reason_str) => Description::try_from(reason_str).map_err(|e| {
                    Error::for_user(
                        Kind::InvalidInput,
                        format!(
                            "There is an issue with the reason '{}'. {}",
                            reason_str, e.message
                        ),
                    )
                }),
                None => Err(Error::for_user(
                    Kind::InvalidInput,
                    "Please provide a reason for the action.".to_string(),
                )),
            },
        }
    }
    ...
    fn validate_signed_test_file(&self) -> Result<SignedFile, Error> {
        match &self.signed_test {
            ...
            None => {
                ...
                let valid_signature = Signature::try_from(file_signature).map_err(|e| {
                    Error::for_user(
                        Kind::InvalidInput,
                        format!(
                            "There is an issue with the test file signature '{}'. {}",
                            file_signature, e.message
                        ),
                    )
                })?;
                SignedFile::new(file_path, &valid_signature).map_err(|e| {
                    Error::for_user(
                        Kind::InvalidInput,
                        format!(
            ...
            None => {
                ...
                let valid_signature = Signature::try_from(file_signature).map_err(|e| {
                    Error::for_user(
                        Kind::InvalidInput,
                        format!(
                            "There is an issue with the evidence file signature '{}'. {}",
                            file_signature, e.message
                        ),
                    )
                })?;
