...
impl Kind {
    pub fn new(kind_value: &str) -> Result<Kind, Error> {
        ...
        for (kind_enum, human_readable_kind) in KIND_STRINGS {
            if kind_value == *human_readable_kind {
                return Ok(kind_enum.clone());
            }
        }
        let list_of_valid_kind_inputs: Vec<&str> =
            KIND_STRINGS.iter().map(|(_, nid_str)| *nid_str).collect();
        Err(Error::for_user(
            crate::error::Kind::InvalidInput,
            format!(
                "'{}' is not a valid Kind. Must be one of: [{}].",
                kind_value,
                list_of_valid_kind_inputs.join(", ")
            ),
        ))
    }
}
