...
fn builder_success() {
    ...
    assert!(
        result.is_ok(),
        "An error was not expected, but was returned: {:?}",
        result.err().unwrap()
    );
    ...
    assert_eq!(
        ...
        Description::try_from("value").unwrap()
    );
...
fn builder_multiple_metadata_activities_actions_and_additional_information_success() {
    ...
    assert_eq!(
        ...
        Description::try_from("value2").unwrap()
    );
    assert_eq!(report.subject().nrn.value, "nrn:procedure:example");
    assert_eq!(report.subject().id.value, "somesubjectid");
    assert_eq!(report.procedure().repository, "https://some-location.com");
...
fn use_functions_override_success() {
    ...
    overriding_metadata
        .add("override-key-2", "value 2")
        .unwrap();
    overriding_metadata
        .add("override-key-3", "value 3")
        .unwrap();
    overriding_metadata
        .add("override-key-4", "value 4")
        .unwrap();

    let overriding_subject =
        Subject::try_new("nrn:sourcecode:override/subject", "overridesubjectid").unwrap();
    let overriding_procedure = Procedure::try_new(
        "https://some-override-location.com",
        "some/override/location",
    )
    .unwrap();

    let test_file_sig = SignedFile::new(
        "./some-location/file.txt",
        &Signature::try_new(SHA256, "the-signature").unwrap(),
    )
    .unwrap();
    let evidence_file_sig = SignedFile::new(
        "./some-location/file.txt",
        &Signature::try_new(SHA256, "the-signature").unwrap(),
    )
    .unwrap();
    ...
    let overriding_activities = Activities::builder()
