...
fn add_activity_success() {
    ...
    assert!(
        result.is_ok(),
        "{}",
        format!("An error was not expected: {:?}", result.err())
    );
...
fn merge_duplicate_activities_without_duplication_success() {
    ...
    let copy_action_1 = Action::builder()
        .name("action-1")
        ...
        .try_build()
        .unwrap();
    let copy_action_2 = Action::builder()
        .name("action-2")
...
fn add_duplicate_activity_to_existing_activity_without_duplication_success() {
    ...
    let activities = activities
        .add_activity("activity-1", &copy_action_1)
        .unwrap();
...
fn add_activity_error() {
    ...
    assert!(
        ...
        "{}",
        format!(
            "An error was expected but one was not returned: {:?}",
            result.unwrap()
        )
    );
