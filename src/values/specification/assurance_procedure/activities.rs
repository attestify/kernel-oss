...
impl Activities {
    ...
    pub fn add(
        ...
        let activity = Activity::new(activity_name, short_desc, long_desc).map_err(|error| {
            Error::for_user(
                Kind::InvalidInput,
                format!(
                    "We could not add the activity '{}' to the list of activities: {}",
                    activity_name, error
                ),
            )
        })?;
    ...
    pub fn merge(&self, activity: &Activity) -> Activities {
        ...
        let position = new_activities
            .list
            .iter()
            .position(|existing_activity| existing_activity.name == activity.name);
        match position {
            Some(index) => {
    ...
    pub fn add_activity(&self, activity_name: &str, action: &Action) -> Result<Activities, Error> {
        ...
        let position = new_activities
            .list
            .iter()
            .position(|existing_activity| existing_activity.name.value == activity_name);
